# Rodrigo Vieira

Computer Graphics and Multimedia Engineering student at ESTG - IPVC, Viana do Castelo.
I bridge the gap between backend development and user interfaces, and I love seeing both sides work perfectly together.

📫 rodrigo.vieira@exemplo.com

---

### Technologies

<!-- Languages -->
**Languages**  
Java · JavaScript · TypeScript · HTML · CSS · SQL

<!-- Frameworks and Tools -->
**Frameworks & Tools**  
Angular · Ionic · Node.js · Express · MongoDB · Firebase · JavaFX · Git · Postman · Figma

<!-- Systems and Networks -->
**Systems & Networks**  
Linux (Postfix, Dovecot) · Network Configuration

---

### Featured Projects

| Project | Description | Stack |
|---|---|---|
| [Multiplayer Reversi](https://github.com/vieira3030/Reversi_LdP_G05) | Networked board game with GUI and saveable matches | Java · JavaFX · Sockets |
| [Olheiro Pro](#) | Web app for searching, comparing, and cataloging football players | Angular · TypeScript |
| [OsMeusLivros](#) | Mobile app for personal library management | Ionic · Angular |
| [EuGaranto](#) | Warranty management app, from user research to prototype | Firebase · Capacitor · Figma |
| [REST API](#) | MVC architecture backend, cloud database, and Swagger documentation | Node.js · Express · MongoDB |
| [Soundboard](https://github.com/vieira3030/2026-ecgm-eds-ab3-soundboard) | Interactive soundboard, developed in a team using sprints | JavaScript · GitHub Pages |

---

### Statistics

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=vieira3030&show_icons=true&hide_border=true&count_private=true&theme=transparent" alt="GitHub Stats" width="48%">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=vieira3030&layout=compact&hide_border=true&theme=transparent" alt="Top Languages" width="48%">
</div>
